...
def train_lora(noise_scheduler, unet, dataloader: DataLoader):
    ...
    for _ in range(NUM_EPOCHS):
        ...
        for batch in tqdm(dataloader):
            ...
            encoder_hidden_states = batch["encoder_hidden_states"].to(device).squeeze(1)
...
def main():
    ...
    # it will be trained with LoRa, so freeze unet
    unet.requires_grad_(False)

    # create and config LoRa
    lora_config = LoraConfig(
        r=4,
        lora_alpha=4,
        target_modules=["to_q", "to_v", "to_k", "to_out.0"], # Capas de atención
        lora_dropout=0.01,
        bias="none",
    )
    ...
    # train LoRa
    unet = train_lora(noise_scheduler=noise_scheduler, unet=unet, dataloader=train_dataloader)
    unet.save_pretrained("./lora")
